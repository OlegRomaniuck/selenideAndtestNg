 ...
 * Created by Tester3
 */
public class JsonSchemaAndStatusValidator {

    public static final String JSON_V4_SCHEMA_IDENTIFIER = "http://json-schema.org/draft-04/schema#";
    public static final String JSON_SCHEMA_IDENTIFIER_ELEMENT = "$schema";

    public String validStatusAndGetData(String jsonFile, String path) {
        return Stream.of(jsonFile)
                .filter(s -> {
                    if (from(s).getString(path) != null && !from(s).getString(path).equals("[]")) {

                        System.out.println("TRUE IN STREAM");
                        return true;
                    } else {
                        System.out.println("FALSE IN STREAM");
                        return false;
                    }
                }).map(s -> from(s).getString(path))
                .findFirst().orElse("NO DATA");

    }

    public boolean validStatusAndJson(File schemaFile, String jsonFile, String path, String code) {
        boolean valid = false;
        try {
            valid = isJsonValid(schemaFile, jsonFile);
        ...
        if (valid) {
            ...
            if (from(jsonFile).getString(path) != null && from(jsonFile).getString(path).equals(code)) {
                return true;
            }
        }
        return false;
    ...
    public static boolean isJsonValid(File schemaFile, String jsonFile) throws IOException, ProcessingException {
        final JsonSchema schemaNode = getSchemaNode(schemaFile);
        final JsonNode jsonNode = getJsonNode(jsonFile);
        return isJsonValid(schemaNode, jsonNode);
        // my validations
